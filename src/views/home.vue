...
<template>
    <div class="layout">
                ...
                <div v-for="(item,index) of menu" :key="index">
                    <div v-if="item.children.length>0">
                        <Submenu :name="index">
                            ...
                            <MenuItem v-for="(list,index) of item.children" :key="index" :name="list.path">{{list.name}}</MenuItem>
                        </Submenu>
                    </div>
                    <div v-else>
        ...
        <Layout :style="{marginLeft: '200px'}">
            <Header :style="{ background: '#fff',fontSize:'30px',textAlign:'center', boxShadow: '0 2px 3px 2px rgba(0,0,0,.1)'}">后端管理系统</Header>
